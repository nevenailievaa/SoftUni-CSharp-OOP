        ...
        public void Run()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    if (input[0] == "AddUnit")
                    {
                        string unitName = input[1];
                        string planetName = input[2];


                        string result = this.controller.AddUnit(unitName, planetName);
                        this.writer.WriteLine(result);
                    }
                    if (input[0] == "AddWeapon")
                    {
                        ...
                        string result = this.controller.AddWeapon(planetName, weaponName, destructionLevel);
                        this.writer.WriteLine(result);
                    }
